...
module BUFFER_WRAPPER_SR #(
    ...
    // 不同bank所属打拍寄存器的下标位宽
    localparam BANK_IDX_W     = $clog2(ARRAY_SIZE);
    ...
    // 用写计数器算出的 bank 号作为写寄存器下标，对于不同的array_size，取bank号的低BANK_IDX_W位
    ...
    // 同样，写权重矩阵的时候也直接取wei_target_bank作为寄存器编号
    ...
    // 每个逻辑 bank 一份写数据/地址/使能寄存器。
    // unpacker 输出不再直接扇出到 8*BANK_DEPTH 个 ram_reg 的 D 端。
    // 物理写入晚 1 拍：本拍用 wr_cnt 算出 bank/addr 并装入 *_q，下一拍 RAM 才写。
    logic [MATRIX_DW-1:0]  act_wdata_q [0:ARRAY_SIZE-1];
    logic [ADDR_WIDTH-1:0] act_waddr_q [0:ARRAY_SIZE-1];
    logic                  act_wren_q  [0:ARRAY_SIZE-1];
    logic [MATRIX_DW-1:0]  wei_wdata_q [0:ARRAY_SIZE-1];
    logic [ADDR_WIDTH-1:0] wei_waddr_q [0:ARRAY_SIZE-1];
    logic                  wei_wren_q  [0:ARRAY_SIZE-1];
    ...
    always_ff @(posedge pclk or negedge prst_n) begin
        if (!prst_n) begin
            ...
            if (do_act_write) begin
                act_wdata_q[act_bank] <= current_unpack_data;
                act_waddr_q[act_bank] <= act_wr_addr_val;
                act_wren_q[act_bank]  <= 1'b1;
            end
            if (do_wei_write) begin
                wei_wdata_q[wei_bank] <= current_unpack_data;
                wei_waddr_q[wei_bank] <= wei_wr_addr_val;
                wei_wren_q[wei_bank]  <= 1'b1;
            end
        end
    end
    ...
    generate
        for (i = 0; i < ARRAY_SIZE; i = i + 1) begin : sram_inst_block
            ...
            simple_dualport_ram #(
                ...
                // [EDIT] 接打拍后的写口，不再接 current_unpack_data / act_wr_addr_val
                .wr_addr  (act_waddr_q[i]),
                .wren     (act_wren_q[i]),
                .wr_data  (act_wdata_q[i])
            );
            ...
            simple_dualport_ram #(
                ...
                // [EDIT]
                .wr_addr  (wei_waddr_q[i]),
                .wren     (wei_wren_q[i]),
                .wr_data  (wei_wdata_q[i])
            );
        end
    endgenerate
